...
def parse_card_data(card_df):
    ...
    parsed_data["type"]  = parse_type(card_df["type"])
    parsed_data["race"] = parse_race(card_df["race"])
    parsed_data["attribute"] = parse_attribute(card_df["attribute"])
    return parsed_data

def check_extra(card_df):
    isExtra = False
    parsed_type  = parse_type(card_df["type"])
    for pt in parsed_type:
        if pt[0] in ["融合","シンクロ", "エクシーズ","リンク"]:
            isExtra = True
    return isExtra